...
def home():
    return {
        ...
        "endpoints": {
            "receber_pix": "POST /rota-recebimento/pix/pix",
...
@app.post("/rota-recebimento/pix/pix")
...
def consulta_maquina1():
    global valor_do_pix
    pulsos =formata_valor(valor_do_pix)
    valor_do_pix = 0
...
def consulta_maquina2():
    ...
    pulsos = formata_valor(valor_pix_maquina2)
    valor_pix_maquina2 = 0
    return {"retorno": pulsos}

def formata_valor(valor: float, ticket: float = 1.0) -> str:
    """Formata o valor em múltiplos de  R$1, com 4 dígitos (ex: '0005')"""
    if valor > 0 and valor >= ticket:
        retorna_maquina_shopp = int(valor // ticket)
        return f"{retorna_maquina_shopp:04d}"
    return "0000"
